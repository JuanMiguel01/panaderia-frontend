...
const PRESETS = [
  ...
  { id: 7,  name: 'Pan Perro Chico', price: 120, emoji: '🌭' },
  { id: 8,  name: 'Pan Molde',       price: 310, emoji: '🍞' },
  { id: 9,  name: 'Pan Hambur',      price: 290, emoji: '🍔' },
  { id: 10, name: 'Pan Flauta',      price: 140, emoji: '🥖' },
];
...
export function AddBatchForm({ onCreateBatch }) {
  ...
  const [quantities, setQuantities] = useState({});   // { presetId: '200' }
  const [customRows, setCustomRows] = useState([]);   // [{ tmpId, name, price, quantity }]
  const [isLoading, setIsLoading]   = useState(false);

  // ── Cantidad de un preset ─────────────────────────────
  const setQty = (id, val) => {
    setQuantities(prev => {
      if (val === '' || val === '0') {
        const copy = { ...prev };
  ...
  const addCustomRow = () =>
    setCustomRows(prev => [...prev, { tmpId: Date.now(), name: '', price: '', quantity: '' }]);

  const updateCustomRow = (tmpId, field, val) =>
    setCustomRows(prev => prev.map(r => r.tmpId === tmpId ? { ...r, [field]: val } : r));

  const removeCustomRow = (tmpId) =>
    setCustomRows(prev => prev.filter(r => r.tmpId !== tmpId));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const all = [
        ...selectedPresets.map(p => ({ breadType: p.name, quantityMade: Number(quantities[p.id]), price: p.price, date })),
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-amber-50/50">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">Ingresa la cantidad producida de cada pan</p>
          </div>
        </div>
        ...
        <div className="px-5 py-4 space-y-5">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {date !== todayISO() && (
              <>
                ...
                <button type="button" onClick={() => setDate(todayISO())} className="text-xs text-amber-600 underline hover:text-amber-700">
                  Usar hoy
                </button>
              </>
            )}
          ...
          <div>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">
              Selecciona e ingresa cantidades
            </p>
            ...
            <div className="space-y-2">
              ...
              {customRows.map(row => (
                <div key={row.tmpId} className="grid grid-cols-[1fr,100px,90px,36px] gap-2 items-center">
                  ...
                  <button
                    type="button"
                    onClick={() => removeCustomRow(row.tmpId)}
        ...
        <div className="px-5 py-4 border-t border-gray-100 bg-gray-50/60">
          {totalBatches > 0 ? (
            ...
            <p className="text-sm text-gray-400 text-center py-1">
              Ingresa la cantidad de al menos un tipo de pan
            </p>
          )}
        </div>
      </form>
...
function PresetCard({ preset, qty, active, onChange }) {
  ...
  return (
    <div
      onClick={focusInput}
      className={`
        relative rounded-xl border-2 p-3 transition-all duration-150 cursor-pointer select-none
        ${active
    ...
    >
      ...
      <p className="text-xs text-gray-400 mb-2">${preset.price}</p>
      ...
      <input
        ...
        className={`
          w-full text-center text-sm font-bold rounded-lg px-2 py-1.5 border outline-none transition-all
          ${active
